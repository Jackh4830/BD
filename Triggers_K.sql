...
DELIMITER $$
	CREATE TRIGGER activeTableEstadoPago AFTER UPDATE ON pago 
		FOR EACH ROW
			BEGIN
...
DELIMITER &&  
	CREATE TRIGGER totalPago AFTER INSERT ON  actualizarPago-- `detalle pedido`
		FOR EACH ROW 
			BEGIN
				SET @pedidoSimulacionPagoBoton=(SELECT pedido FROM simulacionPagoBoton ORDER BY pk DESC LIMIT 1);
                SET @updateTotalPago = (SELECT SUM(DPE_subtotal) FROM `detalle pedido` WHERE PEDIDO_PED_id = @pedidoSimulacionPagoBoton);
				UPDATE PAGO SET PAG_total=@updateTotalPago WHERE PEDIDO_PED_id = @pedidoSimulacionPagoBoton;
			END; 
	&&
DELIMITER ;
select @pedidoSimulacionPagoBoton;
set @pedidoSimulacionPagoBoton = 'nada';
select @updateTotalPago;
set @updateTotalPago = 0;
...
-- #6 OK: LLENAR TABLA PAGO
DROP TRIGGER IF EXISTS llenandoTablaPago;
DELIMITER $$
	CREATE TRIGGER llenandoTablaPago AFTER INSERT ON simulacionPagoBoton
		FOR EACH ROW
			BEGIN				
                SET @comodinPKpago = '-pag'; 
                SET @pkPago = 0;
                SET @pkPago = @pkPago + 1;
                SET @ultimaFKPedido = (SELECT PEDIDO_PED_id  FROM subtotal ORDER BY DPE_fecha DESC LIMIT 1);                
				INSERT INTO pago VALUES (concat(@pkPago,@comodinPKpago),'ESPERA', now(),0, @ultimaFKPedido);
                INSERT INTO actualizarpago (estado, fkPedido) VALUES ("actualice", @ultimaFKPedido);
            END;
    $$
DELIMITER ;
...
SELECT * FROM `banca cliente` WHERE BANCLI_saldo=62552; 
SELECT * FROM pedido;
SELECT * FROM cliente;
select @text;
